...
//     });
    ...
    function initForNonMobileCompare() {
        $('body').on('click', '.compare-cards-list .onoffswitch-checkbox', function (e) {
            var me = $(e.target),
                ...
                cardAdded = compare_cards_result.find('.card-item').not('.card-add-plus'),
                cardType = cardItem.data('type');
            
            if (cardAdded.length === 4 && !compare_cards_result.find('[data-type="'+cardType+'"]').length) {
                me.prop('checked', false);
                return;
            }

            if (cardType) {
                $('[data-type="'+cardType+'"]').toggleClass('active');
                if ($(this).prop('checked')) {
                    $('[data-type="'+cardType+'"]').find('input').prop('checked', true);
                } else {
                    $('[data-type="'+cardType+'"]').find('input').prop('checked', false);
                }
            } else {
                cardItem.toggleClass('active');
            }

            if (compare_cards_result.find('[data-type="'+cardType+'"]').length) {
                var duplicateCard = compare_cards_result.find('[data-type="'+cardType+'"]');
                duplicateCard.html(card_add_plus_hidden.html());
                duplicateCard.attr('class', 'card-item card-add-plus');
                duplicateCard.attr('data-type', null);
            } else {
                ...
                if (cardAddplus.length > 0) {
                    ...
                    cardAddplus.attr('data-type', cardType);
    ...
    function initForMobileCompare() {
        $('body').on('click', '.compare-cards-list .onoffswitch-checkbox', function (e) {
            ...
            if (cardAdded.length == 2 && !compare_cards_result.find('[data-type="'+cardType+'"]').length) {
                me.prop('checked', false);
                return;
            }
            if (cardType) {
    ...
    $('#compare-card-action, #compare-card-action-mobile').click(function () {
        ...
        compare_cards_result.fadeIn(function(){
            ...
            if (isMobile.phone) {
                filterCardsMobile.css('display', 'none');
            }
