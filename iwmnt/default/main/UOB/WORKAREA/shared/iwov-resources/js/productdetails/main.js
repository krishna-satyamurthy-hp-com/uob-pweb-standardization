...
jQuery(function($) {
    ...
    function initApplynowFixed() {
        ...
        if (applyFixed.length > 0) {
            ...
            $('.arrow-expand').on('click', function() {
                var me = $(this);
                if (!me.hasClass('open')) {
                    me.addClass('open');
                    element.slideDown();
    ...
    function initImageBlock() {
        if($('.image-item-wrap').length > 0){
            ...
            if(!isMobile.phone){
                $('.image-item-wrap .content-text').setAllToMaxHeight();
                $('.image-item-wrap.type-card .content-text').css('height', 'auto');
                $('.image-item-wrap').setAllToMaxHeight();
            }
            if(isMobile.phone){
                $('.image-item-wrap').setImageBlockToMaxHeight();
            }
        }
    }
