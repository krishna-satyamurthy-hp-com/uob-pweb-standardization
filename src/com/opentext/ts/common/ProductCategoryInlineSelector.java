...
public class ProductCategoryInlineSelector {
	private static final Log LOGGER = LogFactory
			.getLog(ProductCategoryInlineSelector.class);

	public static void main(String path[]) throws Exception {
		LOGGER.debug("Entering ProductCategoryInLineSelector::execute");
		try {
			...
			List l = doc
					.selectNodes("//root/product_category_details/product_category_name");
			for (Iterator ite = l.iterator(); ite.hasNext();) {
				Node n = (Node) ite.next();
				option.append("<option value=").append("\"")
						.append(n.getText()).append("\"").append(" ")
						.append("label=\"").append(n.getText()).append("\"/>")
						.append("\n");
			}
